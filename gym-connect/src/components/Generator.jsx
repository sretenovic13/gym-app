    ...
    return (
        <div className='flex flex-col gap-4'>
            <div className='flex justify-center items-center gap-2'>
                <p className='text-3xl sm:text-4l md:text-5xl font-semibold text-slate-400'>{index}</p>
            ...
            {Object.keys(WORKOUTS).map((type, typeIndex) => {
                return (
                    <button onClick={() => {
                        setMuscles([])
                        setPoison(type)
                    }} key={typeIndex} className={'bg-slate-950 border px-4 duration-200 px-4 hover:border-blue-600 py-3 rounded-lg ' + (type === poison ? ' border-blue-600' : ' border-blue-400')}
                    >
        ...
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
            {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
                return (
                    <button onClick={() => {
                        setGoal(scheme)
                    }} className={'bg-slate-950 border px-4 duration-200 hover:border-blue-600 py-3 rounded-lg px-4 ' + (scheme === goal ? ' border-blue-600' : ' border-blue-400')}
                    key={schemeIndex}>
